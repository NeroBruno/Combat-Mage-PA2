...
public class SpawnProjectilesScript : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetKey(KeyCode.Space) && Time.time >= timeToFire || Input.GetMouseButton(0) && Time.time >= timeToFire)
        {
            ...
            SpawnVFX(); // THIS IS THE RELEVANT INFO TO SPAWN SPELLS
        }
    ...
    // THIS IS WHAT I WANT TO USE ON THE PLAYER ATTACK SPELL STILL NEED TO CHECK IF I CAN LEAVE THE CURRENT LOGIC OR WILL HAVE TO SEPARATE EVERYTHING
